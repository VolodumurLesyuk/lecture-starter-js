...
function updateFightStatus(selectedFighters) {
	...
	// Оновлення індикатора здоров'я першого бійця
	const leftFighterHealth = Math.max(0, firstFighter.health); // Гарантуємо, що значення здоров'я не буде від'ємним
	...
	// Оновлення індикатора здоров'я другого бійця
	const rightFighterHealth = Math.max(0, secondFighter.health); // Гарантуємо, що значення здоров'я не буде від'ємним
  ...
  function endFight(winner) {
	...
	if (winner) {
	  ...
	  winnerContainer.classList.add('fullscreen'); // Додати клас для розширення на весь екран
	  setTimeout(() => {
		location.reload(); // Перезавантажити сторінку через 5 секунд
	  }, 5000);
	} else {
...
async function renderArena(selectedFighters) {
  ...
  function handleKeyDown(event) {
	...
	if (event.code === controls.PlayerOneAttack) {
  	// Дії при натисканні клавіші атаки першого гравця
	...
	console.log(selectedFighters)
  	...
  	// Дії при натисканні клавіші блоку першого гравця
  	// Наприклад, збільшення оборони першого бійця
  	firstFighter.defense += 10;
	} else if (event.code === controls.PlayerTwoAttack) {
  	// Дії при натисканні клавіші атаки другого гравця
  	...
  	// Дії при натисканні клавіші блоку другого гравця
  	// Наприклад, збільшення оборони другого бійця
  	secondFighter.defense += 10;
	} else if (controls.PlayerOneCriticalHitCombination.includes(event.code)) {
  	// Дії при натисканні комбінації критичного удару першого гравця
  	...
  	// Дії при натисканні комбінації критичного удару другого гравця
	...
	// Перевірка стану бійців після дій
	...
	// Перевірка переможця та закінчення бою
	if (winner) {
		winnerContainer.style.display = 'block';
		endFight(winner);
		document.removeEventListener('keydown', handleKeyDown); // Видаляємо обробник події після закінчення бою
	}
	else {
